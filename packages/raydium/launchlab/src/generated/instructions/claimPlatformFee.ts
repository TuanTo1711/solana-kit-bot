...
export const CLAIM_PLATFORM_FEE_DISCRIMINATOR = new Uint8Array([156, 39, 208, 135, 76, 237, 61, 72])
...
> {
  ...
  if (!accounts.recipientTokenAccount.value) {
    accounts.recipientTokenAccount.value = await getProgramDerivedAddress({
      ...
      seeds: [
        getAddressEncoder().encode(expectAddress(accounts.platformFeeWallet.value)),
        getBytesEncoder().encode(
          new Uint8Array([
