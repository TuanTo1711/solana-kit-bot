...
import {
  ...
  type PoolKeys,
  type PumpswapClient,
} from '@solana-kit-bot/pumpswap-program'
...
export class TradingOperations {
  ...
  async executeBuyPhase(
    ...
    const createBuyTransaction = async (signer: any, index: number) => {
      ...
      const swapResult = computeBuyQuoteIn({
        quote: amountIn,
        slippage: 1,
        baseReserve: this.poolMonitor.baseReserve,
        quoteReserve: this.poolMonitor.quoteReserve,
        coinCreator: poolKeys.coinCreator,
      })
      const { maxQuote, base } = swapResult

      // Create buy instructions
      const buyInstructions = await this.client.createBuyInstructions(
        {
        ...
        { hasBaseAta: true, hasQuoteAta: false }
      )
      ...
      console.log(`Ví ${index + 1}: giá=${priceResult.price}, mua=${amountIn}, nhận=${base}`)
  ...
  async executeSellPhase(
    ...
    const createSellTransaction = async ([signerAddress, tradeData]: [string, SignerTradeInfo]) => {
      ...
      const sellInstructions = await this.client.createSellInstructions(
        {
          amountIn: amountOut,
          minAmountOut: 0n,
          poolKeys,
        ...
        { hasQuoteAta: false, sellAll: false }
      )
