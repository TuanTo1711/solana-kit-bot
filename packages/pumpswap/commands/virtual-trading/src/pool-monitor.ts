...
import type { PoolKeys } from '@solana-kit-bot/pumpswap-program'
...
export class PoolMonitor {
  ...
  async subscribeToPoolUpdates(context: SolanaBotContext, poolKeys: PoolKeys): Promise<void> {
    ...
    const [baseObservable, quoteObservable] = await Promise.all([
      this.createTokenObservable(context, poolKeys.poolBaseTokenAccount),
      this.createTokenObservable(context, poolKeys.poolQuoteTokenAccount),
    ])
