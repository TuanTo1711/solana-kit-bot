...
export class PoolCheckerConfigCommand extends Command<BaseContext & PoolCheckerConfigContext> {
  ...
  async view() {
    const configs = await this.context.db.config.findMany()
    if (configs.length === 0) {
  ...
  async add() {
    ...
    await this.context.db.config.create({ data })
  ...
  private async edit() {
    ...
    if (config) {
      ...
      await this.context.db.config.update({
        data: updatedConfig,
...
PoolCheckerConfigCommand.paths = [['config']]
